...
def filter_nav_groups_for_user(nav_groups, guest=False, hr_base=False):
    if guest:
...
def inject_maxek_layout():
    ...
    visible_nav_groups = filter_nav_groups_for_user(
        NAV_GROUPS, guest=guest_user, hr_base=hr_base_user
    )
    try:
...
def render_choice_b_dashboard():
    ...
    workflow_preview = get_user_workflow_preview(db, user_id, admin, limit=5)
    user_pending_count = count_user_pending_workflows(db, user_id, admin)
    system_rows = build_system_rows(user_id=user_id, admin=admin, limit=5)
    total_pending = (
    ...
    if request.method == "POST":
        ...
        if existing_staff:
            db.execute(
                ...
                "working_hours=?, joining_date=?, photo=?, status=?, "
            ...
            db.execute(
                ...
                "ot_applicable, ot_rate_per_hour, holiday_pay_applicable, working_hours, joining_date, photo, status, "
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if action == "save_branch" and redirect_cid:
                ...
                save_branch(db, request.form, session.get("username", ""), bid)
                db.commit()
                flash("Branch saved.")
...
USER_MGMT_SYSTEM_ROLES = ("User", "Guest", "Admin")
...
def user_management():
    ...
    if request.method == "POST":
        ...
        if system_role not in USER_MGMT_SYSTEM_ROLES:
        ...
        try:
            db.execute(
                "INSERT INTO users(username, password, role, workflow_role, employee_name, status) "
                "VALUES(?,?,?,?,?,?)",
                (
                    username,
...
def store():
    ...
    modules = [
        ...
        {"endpoint": "purchase_vendors", "label": "Vendor Master", "icon": "fa-truck-field", "description": "GST/PAN, contacts, documents"},
...
def purchase_vendors():
    ...
    _prepare_store_db(db)
    view_id = request.args.get("view", type=int)
    view_vendor = get_vendor(db, view_id) if view_id else None
    view_docs = list_vendor_documents(db, view_id) if view_vendor else []
    if request.method == "POST":
        try:
            vid = request.form.get("vendor_id", "").strip()
            vendor_id = save_vendor(db, request.form, int(vid) if vid else None)
            upload = request.files.get("vendor_doc")
            if upload and upload.filename:
                stored = save_file(upload, STORE_DOCS_DIR)
                if stored:
                    save_vendor_document(
                        db,
                        vendor_id,
            ...
            db.commit()
            flash("Vendor saved.")
            return redirect(url_for("purchase_vendors", view=vendor_id))
        except (ValueError, sqlite3.IntegrityError) as exc:
...
def vendor_document_download(vendor_id, filename):
    safe = os.path.basename(filename)
    path = os.path.join(STORE_DOCS_DIR, safe)
    if not os.path.isfile(path):
        abort(404)
    return send_from_directory(STORE_DOCS_DIR, safe, as_attachment=True)
...
def client_billing_import_dpr():
    ...
    try:
        lines = import_dpr_measurements(db, int(project_id), period_from, period_to, bill_id)
