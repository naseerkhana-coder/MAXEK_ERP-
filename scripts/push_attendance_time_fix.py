"""Deploy flexible attendance time entry to VPS."""
...
def main() -> None:
    ...
    for local_rel, remote_rel in FILES:
        print(f"Upload {ROOT / local_rel} -> {args.remote_dir}/{remote_rel}")
        sftp.put(str(ROOT / local_rel), f"{args.remote_dir}/{remote_rel}".replace("\\", "/"))
    sftp.close()
    _, stdout, stderr = client.exec_command(
        "systemctl restart maxek-erp maxek-api 2>/dev/null; systemctl is-active maxek-erp"
    )
    print((stdout.read() or stderr.read()).decode().strip())
    client.close()
    print("Done. Hard-refresh Attendance (Ctrl+F5).")
