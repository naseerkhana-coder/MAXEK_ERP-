...
_sub_cols = {row[1] for row in cursor.fetchall()}
for _col, _typ in (
    ("subcontractor_id", "TEXT"),
    ("subcontractor_name", "TEXT"),
    ("project_name", "TEXT"),
    ...
    if _col not in _sub_cols:
        cursor.execute(f"ALTER TABLE subcontractors ADD COLUMN {_col} {_typ}")
if "sub_code" in _sub_cols and "subcontractor_id" in _sub_cols:
    cursor.execute(
        """
...
if "sub_name" in _sub_cols and "subcontractor_name" in _sub_cols:
    cursor.execute(
        """
...
if "pan_number" in _sub_cols and "pan_card_number" in _sub_cols:
    cursor.execute(
        """
...
def open_erp():
    ...
    tk.Label(sub_tab, text="Status").grid(row=9, column=0, pady=10, padx=10, sticky="w")
    sub_status = ttk.Combobox(sub_tab, values=["Active", "Inactive"], width=47, state="readonly")
    sub_status.set("Active")
    sub_status.grid(row=9, column=1)

    tk.Label(sub_tab, text="Joining Date").grid(row=10, column=0, pady=10, padx=10, sticky="w")
    sub_date = DateEntry(sub_tab, date_pattern="dd-mm-yyyy")
    sub_date.grid(row=10, column=1)
    ...
    tk.Button(sub_tab, text="SAVE SUB CONTRACTOR", bg="blue", fg="white", command=save_sub).grid(row=11, column=1, pady=20)
